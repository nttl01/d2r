...
pGoatmanXY = (150, 124)  # 염소인간 피바 좌측 끝 지점 - 이거 없다면 죽은것이므로 다시 살려야됨
cPos1 = (502, 994)  # 게임화면인지 확인하는 체크포인트 (피약의 반사처리되는 부분(흰색))
xStartP = 665
...
while True:
    ...
    if mHelperFlag and yRes1 == "red" and yRes2 != "red":
        print(" 용병 피 부족 : ", end=" ")
        if yEmerRes1 != "red":  # 비상상황인지 체크 // red=정상, green=중독
            # 피약 슬롯 확인하여 먹기
            i = 0
            for slotRgb in slotXy:
                i = i + 1
    ...
    # ===== 염소인간 자동 소환 처리
    if mGoatmanFlag:
        if yRes1 == "red" and gRes1 != "red":   #용병이 있을때만 소환을 체크해 실행한다.
            if mGoatmanBuff[0]:
